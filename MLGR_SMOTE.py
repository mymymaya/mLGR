...
def MLSMOTE(X, y, n_sample):
    ...
    for i in range(n_sample):
        ...
        #target[i] = np.array([1 if val >= 2 else 0 for val in ser]) #t1
        # for val in range(7): #t2
        #     if val == 0:
        #         target[i][val]=1 if ser[val] >= 1 else 0
        #     else:
        #         target[i][val]=1 if ser[val]>=2 else 0
        ratio = random.random()
        gap = X.loc[reference, :] - X.loc[neighbour, :]#feature
...
def main(filename,n):
    ...
    X, y = load_dataset(filename,n)

    X_sub, y_sub = get_minority_instace(X, y)  # Getting minority instance of that datframe
    X_res, y_res = MLSMOTE(X_sub, y_sub, 3053)  # Applying MLSMOTE to augment the dataframe
...
if __name__ == '__main__':
    ...
    for e in [400]:
        for b in [16]:
            print(e,b)
            filename = dirpath + str(e)+'_'+str(b)+'_68lbs.txt'
            new_data = main(filename,68)
            output_file = dirpath +str(e)+'_'+str(b)+'_mir10.txt'
